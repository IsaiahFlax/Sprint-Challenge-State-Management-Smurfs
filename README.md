...
- [ ] What problem does the context API help solve?
        ...
        The problem is passing props, and contextApi allows for global state, so no prop drilling.

- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

        Actions are objects that tell reducers what to do.  They have a type, and then whatever other information you want.
	
        Reducers are functions that take state and action as parameters and return a new state.
	
        The store is the single source of truth because it is the only place in the app where state is stored.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

        Application state is global and immutable.  component state is different components having different states.  Component state is good for something simple because it is easier than setting up applications for application state, but application state becomes easier when dealing with larger applications.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

        Action-creators usually return an action, which is just an object.  thunk lets action-creators return functions with dispatch.

- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!

        Context API and Redux seem like they do more-or-less the same thing.  Context API seems easier to understand, but Redux has thunk.  Right now I like redux better than context but we have used redux more.
## Project Set Up
